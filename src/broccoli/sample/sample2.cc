...
namespace broccoli {
  ...
  constexpr static double PBR_HDR_EXPOSURE_BIAS = 10.0f;
}
...
namespace broccoli {
  ...
  void SampleActivity2::draw(broccoli::RenderFrame &frame) {
    ...
    auto sun_animation_progress = std::fmod(engine().currUpdateTimestampSec(), 5.0) / 5.0;
    ...
    double sun_intensity = USE_BLINN_PHONG ? 1.0 : 100.0;
