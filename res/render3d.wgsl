...
fn vertexShaderMain(vertex_input: VertexInput) -> FragmentInput {
  ...
  fragment_input.clip_position = perspectiveProjection(cam_position);
...
@fragment
fn fragmentShaderMain(fragment_input: FragmentInput) -> @location(0) vec4f {
  var result = blinnPhongAmbient(fragment_input, u_light.ambient_glow).xyz;
  for (var i = 0u; i < u_light.directional_light_count; i += 1) {
    ...
    result = clamp(result + blinnPhongDir(fragment_input, light_dir, light_color), vec3(0.0), vec3(1.0));
  }
  for (var i = 0u; i < u_light.point_light_count; i += 1) {
    ...
    result = clamp(result + blinnPhongPt(fragment_input, light_pos, light_color), vec3(0.0), vec3(1.0));
  }
...
fn blinnPhongDir(in: FragmentInput, light_dir: vec3<f32>, light_color: vec3<f32>) -> vec3<f32> {
  return
    blinnPhongSpecularDir(in, light_dir, light_color) +
    blinnPhongDiffuseDir(in, light_dir, light_color) +
    vec3<f32>(0.0);
}
fn blinnPhongPt(in: FragmentInput, light_pos: vec3<f32>, light_color: vec3<f32>) -> vec3<f32> {
  return
    blinnPhongSpecularPt(in, light_pos, light_color) +
    blinnPhongDiffusePt(in, light_pos, light_color) +
    vec3<f32>(0.0);
}
fn blinnPhongSpecularDir(in: FragmentInput, light_dir: vec3<f32>, light_color: vec3<f32>) -> vec3<f32> {
...
fn blinnPhongSpecularPt(in: FragmentInput, light_pos: vec3<f32>, light_color: vec3<f32>) -> vec3<f32> {
  let shininess = in.color.w;
  let normal = in.world_normal.xyz;
  let view_pos = u_camera.world_position.xyz;
  let frag_pos = in.world_position.xyz;
...
fn blinnPhongDiffuseDir(in: FragmentInput, light_dir: vec3<f32>, light_color: vec3<f32>) -> vec3<f32> {
  let base_color = in.color.xyz;
  let normal = in.world_normal.xyz;
  let strength = clamp(dot(normal, -light_dir), 0.0f, 1.0f);
  return strength * base_color;
}
fn blinnPhongDiffusePt(in: FragmentInput, light_pos: vec3<f32>, light_color: vec3<f32>) -> vec3<f32> {
  let frag_pos = in.world_position.xyz;
  let light_dir: vec3<f32> = normalize(light_pos - frag_pos);
  return blinnPhongDiffuseDir(in, light_color, light_pos);
}
fn blinnPhongAmbient(in: FragmentInput, ambient_glow: f32) -> vec3<f32> {
